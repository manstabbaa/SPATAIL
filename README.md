...
SCENE — BLENDER is the constructor/animator (imports Meshy GLB, splits parts,
   ...
   ▼
PHONE — plays it (ios/Spatail: clips / sequence / triggers contract;
...
| `ios/Spatail/` | The phone app (iOS-first, dual-target iOS/visionOS) |
...
- Phone: build `ios/Spatail` on the Mac (`xcodegen generate` → Xcode → device);
