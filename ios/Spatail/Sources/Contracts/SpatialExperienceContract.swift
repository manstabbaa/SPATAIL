// SpatialExperienceContract.swift — Codable mirror of the SpatialExperienceContract
// emitted by pipeline/spatail/experience_contract.js and carried inside
// `experience.delta.payload.experience` (LIVE_BRAIN_SPEC §1 / REALTIME_PROTOCOL §10).
//
// Field-for-field port of SPATAILMobileAR/Models/SpatialContract.swift — the shape
// that decoded the live fusion downlink in production. When the backend schema
// evolves, the changes land here first.
//
// `SourceContent` is the tricky one: the JSON's `sourceContent` is polymorphic across
// content types. Modeled as a struct of optionals so every variant decodes cleanly
...
struct SpatialElement: Codable, Identifiable, Hashable {
    let id: String
    let title: String
    // Raw strings — unknown values must never break decoding; the runtime
    // falls back gracefully at apply time.
