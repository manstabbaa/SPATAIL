// SpatailMaterials.swift — the on-device "master material" effects layer.
// Port of ios/_legacy_Spatail/Sources/SpatailMaterials.swift, verbatim logic.
//
...
//
// TEXTURE-PACK SAFE (Meshy): a Meshy asset arrives as a textured PBR (albedo /
// normal / roughness-metallic / emissive maps baked into the USDZ). Every effect
// mutates the material's channels IN PLACE — `baseColor.tint`,
// `emissiveColor.color` — so the imported MAPS are preserved (tint MULTIPLIES the
// albedo texture rather than replacing it).
//
...
enum SpatailMaterials {
    ...
    /// The SPATAIL highlight accent — matched to the marker so the glow and the
    /// pulsing dot read as one "this part" signal.
    static let accent = UIColor.systemTeal

    /// Non-destructive highlight: keep the surface, add an emissive glow.
    ///
    /// - For a `PhysicallyBasedMaterial` (the usual case from a loaded USDZ, incl.
    ///   a textured Meshy asset) we copy it and raise only the emissive channel in
    ///   place — albedo/roughness/metallic AND all their texture maps survive.
    /// - For a `SimpleMaterial` (placeholder boxes) we rebuild a PBR from its tint
    ///   so it can glow too, since `SimpleMaterial` has no emissive input.
    ...
    /// "Active / energized / powered": a strong self-illumination glow. Same
    /// machinery as `highlighted`, just brighter.
    ...
    /// "State colour": recolour the surface (hot/cold/charged) while keeping the
    /// finish, plus a faint matching glow so the colour reads against AR.
